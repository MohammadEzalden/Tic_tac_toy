...
public class AlphaBeta {
    ...
    public int minimax(char board[][], int depth, boolean isMaxTurn,int alpha,int beta) {

        //write code here

        int evalu=evaluateBoard(board);
        if(evalu==10||evalu==-10||depth>10)return evalu+heuristic(board);
    ...
    public Cell bestMove(char board[][]) {
        ...
        for (int i=0;i<3;i++)
            for (int j=0;j<3;j++){
                if (board[i][j]=='_'){
                    board[i][j]='x';
                    int minmax=minimax(board,0,false,-99999,+99999);
                    if(minmax>best){
                        best=minmax;
                        cell.x=i;
                        cell.y=j;
